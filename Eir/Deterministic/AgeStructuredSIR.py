...
class AgeStructuredSIR(CompartmentalModel):
    ...
    def __init__(
        ...
        self.floatCheck([gamma, S0, I0, R0])
        self.negValCheck([gamma, S0, I0, R0])
        self.lengthCheck([labels, beta[0], gamma, S0, I0, R0])
    ...
    def _deriv(self, s: np.ndarray, i: np.ndarray):
        ...
        # amount leaving S -> I
        x = np.zeros_like(y)
        n_p = self.N / self.N.sum()  # proportion of living in each group
